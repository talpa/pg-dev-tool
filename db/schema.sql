...
begin
    RETURN QUERY
        SELECT (g.col::sdm.t_check_compatibility).global_id,(g.col::sdm.t_check_compatibility).version from (SELECT DISTINCT A.ARR::sdm.t_check_compatibility as  col FROM (SELECT unnest(sdm.array_concat_agg(vd)::sdm.t_check_compatibility[]) AS arr
...
-- zkusim zda existuje verze, pozor, je vazana i na global DISTINCTid
    ...
    RETURN QUERY
        WITH RECURSIVE cte AS (SELECT vd.compatibilities,
             ...
             --na zaver distinct
             cte4 as (
                 SELECT (SELECT ARRAY(SELECT DISTINCT UNNEST(ct3.compatibilities))) AS compatibilities,
                        (SELECT ARRAY(SELECT DISTINCT UNNEST(ct3.global_ids)))           AS global_ids,
...
--
-- Name: array_concat_agg(anyarray); Type: AGGREGATE; Schema: sdm; Owner: -
--

CREATE AGGREGATE sdm.array_concat_agg(anyarray) (
    SFUNC = array_cat,
    STYPE = anyarray
);
